  ...
  return (
    <div className="mx-auto max-w-3xl space-y-5">
      <PageHero
        ...
        title={t("guide.title")}
        size="sm"
        description={<p className="type-body">{t("guide.desc")}</p>}
      />

      <PagePanel>
        <h3 className="type-section text-[length:var(--text-body)]">{t("guide.beforeTitle")}</h3>
        <ul className="mt-3 space-y-2 type-sm type-body-strong">
          {[1, 2, 3].map((n) => (
      ...
      <PagePanel>
        <h3 className="type-section text-[length:var(--text-body)]">{t("guide.runTitle")}</h3>
          ...
          {STEPS.map((step, index) => {
            ...
            return (
              <li key={step.key}>
                ...
                >
                  <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-orange-700/80 text-[length:var(--text-meta)] font-semibold text-white">
                    {index + 1}
                  </span>
                  <div className="min-w-0 flex-1">
                    <p className="type-sm flex items-center gap-2 font-semibold text-white">
                      <Icon className="h-3.5 w-3.5 text-orange-400" />
                      {t(`guide.step.${step.key}.title`)}
                    </p>
                    <p className="type-meta mt-1 text-slate-400">
